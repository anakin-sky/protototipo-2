...
if (!isset($_SESSION['id'], $_SESSION['nombre'], $_SESSION['apellido'])) {
    // Redirigir al inicio de sesión si las variables de sesión no están establecidas
...
// Datos del médico emisor desde la sesión
$nombre_emisor = $_SESSION['nombre'];
$apellido_emisor = $_SESSION['apellido'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['usuario'], $_POST['medicamentos_seleccionados'])) {
        ...
        foreach ($medicamentos_seleccionados as $id_medicamento) {
            ...
            // Si la dosis seleccionada es "nulo", guardar como "no especificado"
            if (empty($dosis) || $dosis === "Nulo") {
                $dosis = "no especificado";
            }

            // Consultar el nombre del medicamento
            $sql_nombre = "SELECT nombre FROM lista_medicamentos WHERE id = $id_medicamento";
            $result_nombre = $conn->query($sql_nombre);
            if ($result_nombre === false || $result_nombre->num_rows === 0) {
                echo "<script>alert('Error al obtener el nombre del medicamento: " . $conn->error . "');</script>";
                exit();
            }
            $row_nombre = $result_nombre->fetch_assoc();
            $nombre_medicamento = $row_nombre['nombre'];

            $sql_insert = "INSERT INTO guardar_receta (id_medicamento, nombre_medicamento, uso_medicamento, id_usuario, usuario, nombre, apellido, cantidad, dosis, frecuencia, fecha_creacion, emisor)
                           SELECT lm.id, lm.nombre, lm.uso, $id_usuario, u.usuario, u.nombre, u.apellido, $cantidad, '$dosis', '$frecuencia', '$fecha_creacion', '$emisor'
        ...
        // Redirigir al listado de recetas del paciente
        header("Location: ../recetasMedico/ver_receta_2.php?id=$id_usuario&fecha=$fecha_creacion");
        exit();
    }
}
...
<head>
    ...
    <style>
        ...
        input[type="submit"] {
            background-color: #4C9FF0; /* Azul claro amigable */
            color: white;
        }

        input[type="submit"]:hover {
            background-color: #3A8AD1; /* Azul más oscuro al pasar el ratón */
        }
    ...
    <main>
        <form action="" method="post" onsubmit="return validarFormulario()">
            ...
            <div class="button-wrapper">
                ...
                <a href="../index.php" class="volver_mi">Volver</a>
            </div>
